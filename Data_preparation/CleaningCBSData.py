...
def load_dataset(filename: str) -> pd.DataFrame:
    """
    Read a CBS CSV file (in DATASETS_DIR) and return it as a DataFrame.
    """
    path = REPO_ROOT / DATASETS_DIR / filename
...
def main():
    ...
    print(
        f"Removed {name_removed_rows} rows that didn't include actual countries.")
    ...
    print(
        f"Removed {counter_party_sector_rows_removed} rows without 'All sectors' in Counterparty sector.")
    print(
    ...
    # recompute quarter cols on the trimmed frame
    quarter_cols = [c for c in Ready_to_split.columns
                    if re.match(r"^\d{4}-Q[1-4]$", c)]

    # with np.errstate(divide='ignore', invalid='ignore'):
    #     Ready_to_split[quarter_cols] = np.log1p(Ready_to_split[quarter_cols])
    ...
    training_columns = [q for q in quarter_cols if 2005 <= int(q[:4]) < 2020]
    testing_columns = [q for q in quarter_cols if int(q[:4]) >= 2020]
    train_df = Ready_to_split[non_time_columns + training_columns]
    test_df = Ready_to_split[non_time_columns + testing_columns]
