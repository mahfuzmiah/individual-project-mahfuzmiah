...
# plt.tight_layout()
# plt.savefig(
#     "/Users/mahfuz/Final_project/Final_repo/Diagrams/PolyMissingPercentage_by_Year.png", dpi=300)
# plt.show()
...
# plt.savefig(
#     "/Users/mahfuz/Final_project/Final_repo/Diagrams/PolyMissingPercentage_by_Reporting_basis.png", dpi=300)
# plt.show()
...
# plt.savefig(
#     "/Users/mahfuz/Final_project/Final_repo/Diagrams/PolyMissingPercentage_by_ReportingCountry.png", dpi=300)
# plt.show()


# # 1) load the cleaned CBS panel
# df = pd.read_csv(
#     "/Users/mahfuz/Final_project/Final_repo/DataSetsCBS/CleanedCBSDataSet.csv")

# 2) identify all the quarterly exposure columns
time_cols = [c for c in df.columns if c.endswith(("-Q1", "-Q2", "-Q3", "-Q4"))]

# 3) compute totals, missing and zeros **without** any prior replacement
total_cells = df.shape[0] * len(time_cols)
missing = df[time_cols].isna().sum().sum()
zeros = (df[time_cols] == 0).sum().sum()

print(f"Total cells: {total_cells}")
print(f"Missing:     {missing}  ({missing/total_cells:.1%})")
print(f"Zeros:       {zeros}  ({zeros/total_cells:.1%})")
